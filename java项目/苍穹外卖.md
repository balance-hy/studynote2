...
> https://www.bilibili.com/read/cv25414453/?spm_id_from=333.999.0.0
...
public Result<PageResult> historyOrders(OrdersPageQueryDTO ordersPageQueryDTO) {
    log.info("历史订单分页查询:{} {} {}", ordersPageQueryDTO.getPage(),ordersPageQueryDTO.getPageSize(),ordersPageQueryDTO.getStatus());
...
public PageResult historyOrders(OrdersPageQueryDTO ordersPageQueryDTO) {
    ...
    Page<Orders> res=orderMapper.pageQuery(ordersPageQueryDTO);
    ...
    return new PageResult(res.getTotal(),orderVOS);
}
```
...
public OrderVO getById(Long id) {
    ...
    Orders orders = orderMapper.getOrder(order);
    if (orders != null) {
        OrderVO orderVO = new OrderVO();
...
```java
Orders getOrder(Orders orders);
```

```xml
<select id="getOrder" resultType="com.sky.entity.Orders">
    select * from orders
    <where>
        <if test="status != null">
            and status = #{status}
...
public void cancel(Long id) {
    //先查询订单 根据订单状态判断是否可以取消
    Orders orders = new Orders();
    orders.setId(id);
    Orders order = orderMapper.getOrder(orders);
    if(order == null){
        //订单不存在 抛出异常
        throw new OrderBusinessException(MessageConstant.ORDER_NOT_FOUND);
    }
...
```java
/**
     * 再来一单 就是把原订单中的商品再加入购物车
     * @param id
     */
@Override
public void repetition(Long id) {
    List<OrderDetail> orderDetails = orderDetailMapper.getByOrderId(id);
    orderDetails.forEach(orderDetail -> { //这里直接转换为购物车中菜品，然后批量插入更好
...
/**
     ...
     * @return
     */
String getOrderDishesToString(Long id);
```
...
public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
    ...
    List<Orders> result = res.getResult();

    if(result !=null && !result.isEmpty()){
        result.forEach(order->{
            OrderVO orderVO = new OrderVO();
            BeanUtils.copyProperties(order,orderVO);
            orderVO.setOrderDishes(getOrderDishesToString(order.getId()));
            orderVOS.add(orderVO);
        });
    }
    return new PageResult(res.getTotal(),orderVOS);
...
/**
     ...
     */
@Override
public String getOrderDishesToString(Long id) {
    List<OrderDetail> orderDetails = orderDetailMapper.getByOrderId(id);
    StringBuffer orderDishesStr = new StringBuffer();
...
* 下单后未支付，订单一直处于“待支付”状态 15分钟之内完成付款
...
public class OrderTask {
    ...
    public void processTimeOutOrder() {
        log.info("定时任务检查订单超时");
        //15分钟不完成支付就算超时 当前时间-15分钟 > 下单时间 说明超时
        LocalDateTime time = LocalDateTime.now().minusMinutes(15);

        List<Orders> orders = orderMapper.getOrderByStatusAndTime(Orders.PENDING_PAYMENT, time);
    ...
    //定时任务检查派送中订单是否完成
    @Scheduled(cron = "0 0 1 * * ?") //每天的凌晨一点检查
    public void processDeliveryOrder() {
        log.info("定时任务检查派送中订单是否完成");
        //每天的凌晨一点检查前一天的派送中订单有无完成
        LocalDateTime time = LocalDateTime.now().minusMinutes(60);

        List<Orders> orders = orderMapper.getOrderByStatusAndTime(Orders.DELIVERY_IN_PROGRESS, time);
...
List<Orders> getOrderByStatusAndTime(Integer status, LocalDateTime time);
```
...
public class ReportServiceImpl implements ReportService {
    ...
    public TurnoverReportVO getTurnoverStatistics(LocalDate begin, LocalDate end) {
        ...
        list.forEach(date -> {
            ...
            Double sum = orderMapper.getSumByTimeAndStatus(map);
            //因为查询出来的sum有可能为空
...
```java
Double getSumByTimeAndStatus(HashMap<String, Object> map);
```
...
<select id="getSumByTimeAndStatus" resultType="java.lang.Double">
...
public UserReportVO getUserStatistics(LocalDate begin, LocalDate end) {
    //begin可以直接计算出来，然后根据查每天新增用户，以及当前用户
    List<LocalDate> list = new ArrayList<>();
    list.add(begin);
    while (!begin.equals(end)) {
        begin = begin.plusDays(1);
    ...
    ArrayList<Integer> oldusers = new ArrayList<>();
    ...
    list.forEach(date -> {
        ...
        map.put("beginTime", beginTime);
        map.put("endTime", endTime);
        Integer sum = userMapper.getSumUser(map);
        newusers.add(sum == null ? 0 : sum);
        //获取旧用户数
        map.remove("beginTime");
        sum=userMapper.getSumUser(map);
        oldusers.add(sum == null ? 0 : sum);
    });

    return UserReportVO.builder()
        .dateList(StringUtils.join(list, ","))
        .newUserList(StringUtils.join(newusers, ","))
        .totalUserList(StringUtils.join(oldusers, ","))
        .build();
}
```
...
/**
     ...
     * @return
     */
Integer getSumUser(HashMap<String, Object> map);
```
...
//后端
{
  ...
  "data": {
    ...
    "orderCompletionRate": 0, //订单完成率
    "orderCountList": "string", //订单数列表
    "totalOrderCount": 0, //订单总数 begin-end订单数
    "validOrderCount": 0, //有效订单数 已完成的订单数
...
public OrderReportVO getOrdersStatistics(LocalDate begin, LocalDate end) {
    //begin可以直接计算出来，然后根据查每天订单数和有效订单数
    List<LocalDate> beginList = getBeginList(begin, end);
    ...
    if(totalOrderCount != 0){
        //注意整型相除，先转换为double
        orderCompleteRate = validOrderCount.doubleValue() / totalOrderCount.doubleValue();
    }
...
//后端
{
  "code": 0,
  "data": {
    "nameList": "string", //商品名称 列表
    "numberList": "string"// 数量 列表
  },
  "msg": "string"
}
```
...
public SalesTop10ReportVO getTop10(LocalDate begin, LocalDate end) {
    //select name,sum(od.number) from order_detail od,orders
    // where od.order_id = orders.id
    // and orders.status=5
    // and orders.order_time between begin and end
    // group by name order by sum(od.number) desc limit 10
...
<select id="getTop10" resultType="com.sky.dto.GoodsSalesDTO">
    select name,sum(od.number) number from order_detail od,orders
    where od.order_id = orders.id and orders.status = 5
    <if test="beginTime != null">
        and order_time &gt; #{beginTime}
    </if>
    <if test="endTime != null">
...
public class WorkSpaceServiceImpl implements WorkSpaceService {
    ...
    public BusinessDataVO businessData() {
        ...
        // newUsers;//新增用户数
        Integer newUsers = userMapper.getSumUser(map);
...
//后端
{
  ...
  "data": {
    "allOrders": 0, //全部订单
    "cancelledOrders": 0, //已取消数量
    "completedOrders": 0, //已完成数量
    "deliveredOrders": 0, //待派送数量
    "waitingOrders": 0 //待接单数量
  },
  "msg": "string"
}
```
...
public OrderOverViewVO overviewOrders() {
    //构建今日时间 beginTime 和 endTime 以及 status=5 的map
...
![image-20240417101537739](https://raw.githubusercontent.com/balance-hy/typora/master/thinkbook/image-20240417101537739.png)

可以看到报表文件还是十分复杂的，如果利用代码去编写无疑很繁琐

所以一般是**自己本地创建好模板文件，程序只负责向其中填写数据**
