...
我们会在稍后的[组件系统](https://v2.cn.vuejs.org/v2/guide/components.html)章节具体展开。不过现在，你只需要明白所有的 Vue 组件都是 Vue 实例，并且接受相同的选项对象 (一些根实例特有的选项除外)。

## 属性和Vue实例

当一个 Vue 实例被创建时，它将 `data` 对象中的所有的 属性 加入到 Vue 的**响应式系统**中。当这些 属性的值发生改变时，视图将会产生“响应”，即匹配更新为新的值。
...
以后你可以在 [API 参考](https://v2.cn.vuejs.org/v2/api/#实例-property)中查阅到完整的实例 property 和方法的列表。
...
> 不要在选项 property 或回调上使用[箭头函数](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Functions/Arrow_functions)，比如 `created: () => console.log(this.a)` 或 `vm.$watch('a', newValue => this.myMethod())`。
>
...
Mustache （双括号）语法不能作用在 HTML attribute 上，遇到这种情况应该使用 [`v-bind` 指令](https://v2.cn.vuejs.org/v2/api/#v-bind)：
...
当 Vue 正在更新使用 `v-for` 渲染的元素列表时，它默认使用“就地更新”的策略。如果数据项的顺序被改变，Vue 将不会移动 DOM 元素来匹配数据项的顺序，而是就地更新每个元素，并且确保它们在每个索引位置正确渲染。这个类似 Vue 1.x 的 `track-by="$index"`。**注意不要使用index，因为要唯一标识，如果数组更新，index也会变。**
...
所以，对于任何复杂逻辑，你都应当使用**计算属性**。**注意调用时，不需要带括号**
...
**结论：方法属性直接调用会每次都执行，计算属性只有当相关依赖改变才重新执行**
...
除了直接绑定到一个方法，也可以在内联 JavaScript 语句中**括号调用方法**：
...
```html
<!-- 阻止单击事件继续传播 也就是所谓事件冒泡，子元素事件触发会导致父元素事件触发-->
...
你可以直接将 [`KeyboardEvent.key`](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/key/Key_Values) 暴露的任意有效按键名来作为修饰符。
...
<!--注意此时选中时，v-model会将对应选框的value值加入checkedNames数组-->
```

据此可以进行总结，当checkedNames为数组时，会将input的value属性对应值加入到数组中，而checkedNames为字符串时会解析“字符串”为布尔值，当有一个checkbox为true时，因为v-model是双向绑定，所以，会返回回去，从而所有checkbox被选中。

详解:

> [vue关于checkbox数据绑定](https://www.cnblogs.com/owenzh/p/11058821.html)
>
...
单选框中，当v-model绑定的是同一个变量，可以产生互斥，省略name属性
...
- 单选：只能选中一个值。
  - v-model绑定的是一个值。
    -当**我们选中option中的一个时，会将它对应的value赋值到selected中,默认value为元素内容**
- 多选：可以选中多个值。
  - v-model绑定的是一个数组。
  - 当**选中多个值时，就会将选中的option对应的value添加到数组selected中，默认value为元素内容**
...
注意当点击按钮时，每个组件都会各自独立维护它的 `count`。因为你每用一次组件，就会有一个它的新**实例**被创建。也就是维护单独的变量空间。
...
然而如果你在模板中尝试这样写，Vue 会显示一个错误(**现在2.7没有显示了**)，并解释道 **every component must have a single root element (每个组件必须只有一个根元素)**。你可以将模板的内容包裹在一个父元素内，来修复这个问题，例如：
...
当点击这个按钮时，我们需要告诉父级组件放大所有博文的文本。幸好 **Vue 实例提供了一个自定义事件的系统**来解决这个问题。父级组件可以像处理 native DOM 事件一样通过 `v-on` 监听子组件实例的任意事件：
...
到这里，你需要了解的解析 DOM 模板时的注意事项——实际上也是 Vue 的全部*必要内容*，大概就是这些了。
...
1. v-on绑定的事件小写加连字符，不要驼峰
2. **methods属性而不是method属性**
3. 组件的template中若要渲染多个，需要给一个根元素！（2.7不会提示这个错误）
...
这不仅为你的组件提供了文档，还会在它们遇到错误的类型时从浏览器的 JavaScript 控制台提示用户。你会在这个页面接下来的部分看到[类型检查和其它 prop 验证](https://v2.cn.vuejs.org/v2/guide/components-props.html#Prop-验证)。
...
## [插槽](https://blog.csdn.net/weixin_72979483/article/details/132701381)

**简单的来说，就是子组件template中预留插槽标签，当使用时，父组件的内容会对插槽标签内容进行替换，若父组件无内容，就用插槽的默认内容填充。**
...
![插槽图示](https://raw.githubusercontent.com/balance-hy/typora/master/2023img/202311050929983.png)
...
***注意：v-slot只能使用在template上，除非只有默认插槽***
...
![具名插槽图示](https://raw.githubusercontent.com/balance-hy/typora/master/2023img/202311050933482.png)
...
**简单点说就是，插槽的内容无法访问到子组件的值，我们通过v-slot在组件模板中传递值给父组件，从而让插槽的内容访问到子组件中的值**
...
![scoped slots diagram](https://raw.githubusercontent.com/balance-hy/typora/master/2023img/202311050937187.svg%252Bxml)
...
<div id="blog-posts-events-demo">
    <current-user>
        <template #default="slotProps">
            {{ slotProps.text }} is {{slotProps.count}}
        </template>
    </current-user>
</div>
